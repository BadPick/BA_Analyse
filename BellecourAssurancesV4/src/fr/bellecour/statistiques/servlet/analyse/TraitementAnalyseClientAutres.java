...
/**
 * Servlet implementation class TraitementAnalyseClientAutres
 */
@WebServlet("/TraitementAnalyseClientAutres")
public class TraitementAnalyseClientAutres extends HttpServlet 
{
     ...
     */
    public TraitementAnalyseClientAutres() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			listeNatures = NatureDao.getAllUtil("autre");
			...
			rd = getServletContext().getRequestDispatcher("/jsp/clientResultatAnalyseAutres.jsp");
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	private String getJsonColumnChart(ArrayList<String[]> resultat, String libelle, boolean cout)
	{
		...
		{
			if (!"Total".equalsIgnoreCase(strings[0]) /*&& !"Non renseigné".equalsIgnoreCase(strings[0])&& !"Non renseignée".equalsIgnoreCase(strings[0])*/)
			{
				json += ",'" + strings[0] + "'";
			}
		}
		...
		{
			if (!"Total".equalsIgnoreCase(s[0]) /*&& !"Non renseigné".equalsIgnoreCase(s[0])&& !"Non renseignée".equalsIgnoreCase(s[0])*/)
			{
			...
			HashMap<String, String> listeAnneesTecn, Date dateDebut, Date dateFin) throws ParseException
	{
		...
		try
		{
			listeNatureBdd = NatureDao.getAllCodeAnalyse("autre");
		...
		for (Sinistre sinistre : listeSinistresFlotte)
		{
			...
			if (sinistre.getNature() == null || sinistre.getNature().getCodeAnalyse() == null ||contenuDansListe==false)
			{
